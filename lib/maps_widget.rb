module MapsWidget
  ...
  def self.format_transit_info(data)
    "<div id='maps-widget' class= 'hidden' style='background-color:rgba(0, 0, 0, 0.6);'><h3 text-align='center' color='white'>Transit</h3><p id='maps'>#{data} <br>#{data.condition.temp}° <br>#{data.condition.text} <br>Wind: #{data.wind.speed}mph <br></p></div>"
  end
